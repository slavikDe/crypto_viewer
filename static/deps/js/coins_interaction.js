    ...
    testButton.addEventListener('click', function () {
        ...
        fetch('/coins/test-coin/', {
            ...
            .then(data => {
                ...
                addButton.disabled = false; // Активуємо кнопку "Add"
                addButton.dataset.market = market; // Зберігаємо дані для кнопки Add
                addButton.dataset.symbol = symbol;
                addButton.dataset.name = name;
            })
            .catch(error => {
                showStatusMessage('Error: ' + error.message, 'red');
                addButton.disabled = true; // Деактивуємо кнопку "Add"
            });
    });
